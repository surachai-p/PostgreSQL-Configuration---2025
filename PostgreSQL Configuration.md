...
<img width="595" height="242" alt="image" src="https://github.com/user-attachments/assets/bbd7b4a9-0a19-4126-a760-7686ac5afc50" />
...
```
### ผลการทดลอง
<img width="445" height="473" alt="image" src="https://github.com/user-attachments/assets/92988d84-cb36-4b61-a24e-22142ae80689" />
...
1. รูปผลการทดลอง
<img width="1202" height="389" alt="image" src="https://github.com/user-attachments/assets/75b3de84-9e64-42f7-9655-9ca4178e24be" />
```
2. อธิบายค่าต่าง ๆ ที่มีความสำคัญ
ตอบ แสดงการตั้งค่าของ Autovacuum ซึ่งเป็นระบบบำรุงรักษาอัตโนมัติที่สำคัญที่สุดของ PostgreSQL
    1: VACUUM (เก็บกวาดข้อมูลเก่า) 🧹
    เมื่อคุณ UPDATE หรือ DELETE ข้อมูลใน PostgreSQL แถวเก่าจะไม่ได้ถูกลบออกไปทันที แต่จะถูกทำเครื่องหมายว่าเป็น "แถวที่ไม่ใช้แล้ว" (dead tuple) หน้าที่ของ
    VACUUM คือเข้ามาเก็บกวาดแถวเหล่านี้เพื่อนำพื้นที่กลับมาใช้ใหม่
    2: ANALYZE (อัปเดตสถิติ) 📊
    เพื่อให้ Query ทำงานได้เร็ว ฐานข้อมูล (Query Planner) ต้องมีข้อมูลสถิติที่ทันสมัยเกี่ยวกับข้อมูลในตาราง (เช่น มีข้อมูลอะไรบ้าง, กระจายตัวอย่างไร) เพื่อจะได้เลือก
    "แผนการทำงาน" (Query Plan) ที่ดีที่สุด หน้าที่ของ ANALYZE
```
...
```
### ผลการทดลอง
<img width="551" height="328" alt="image" src="https://github.com/user-attachments/assets/69118590-67cb-432b-a707-d1b2483e2864" />
...
1. รูปผลการทดลอง
<img width="773" height="219" alt="image" src="https://github.com/user-attachments/assets/ecff5c2b-816b-4f0b-b5ff-84351225a3a1" />

<img width="719" height="229" alt="image" src="https://github.com/user-attachments/assets/b9b6126c-5a1b-4211-a25b-0d49bf3083e9" />

```
2. อธิบายผลลัพธ์ที่ได้
ตอบ มันเออเร่อครับแต่ทำการสร้างตารางไปแล้ว แต่มันบอกว่า ไม่พบตารางที่สร้าง เลยทำการสร้างใหม่แล้วขึ้นดังภาพคับ มีข้อมูลอยู่ 1 row 
```
...
```
### ผลการทดลอง
<img width="642" height="185" alt="image" src="https://github.com/user-attachments/assets/d3b42cf1-8102-4e6a-a824-f022b7a152cb" />
...
```
### ผลการทดลอง
<img width="792" height="515" alt="image" src="https://github.com/user-attachments/assets/5464a9ea-dedf-4a99-9a48-4457ed65fddf" />

<img width="857" height="512" alt="image" src="https://github.com/user-attachments/assets/1d6b2525-6268-40ba-a62a-94b7a39f46e4" />
...
### ผลการทดลองเล็กๆ
<img width="500" height="121" alt="image" src="https://github.com/user-attachments/assets/f5bd8fb1-c481-4627-a2cc-29b3c9144d50" />
<img width="677" height="211" alt="image" src="https://github.com/user-attachments/assets/2ecb5583-6ce5-4ee1-b433-d2b55eced2fd" />

### รูปผลการทดลองขนาดกลาง
1. รูปผลการทดลอง
<img width="705" height="230" alt="image" src="https://github.com/user-attachments/assets/7bea9c40-b540-46ca-855f-9523b0528317" />
...
ตอบ  SELECT (การอ่าน) 🔍
      ...
      Autovacuum (พ่อบ้านอัตโนมัติ) 🤖
      หน้าที่ 1 (ANALYZE): คอยอัปเดต "สารบัญ" หรือสถิติของข้อมูลหลังมีการ INSERT เยอะๆ เพื่อให้ SELECT ทำงานได้ฉลาดขึ้น
      หน้าที่ 2 (VACUUM): คอยมาเก็บกวาด "ขยะ" ที่เกิดจากการ UPDATE และ DELETE เพื่อให้ตารางไม่อ้วนและทำงานเร็วอยู่เสมอ
```

-- ทดสอบหนักขึ้น เครื่องใครไม่ไหวผ่านก่อน หรือเปลี่ยนค่า 500 เป็น 200 :)
SELECT * FROM simulate_oltp_workload(500);
### ผลการทดลอง
<img width="717" height="224" alt="image" src="https://github.com/user-attachments/assets/0ecfd3f6-3245-439f-bc40-683f6e0b3940" />
...
ตอบ shared_buffers ซึ่งทำหน้าที่เป็นแคชสำหรับเก็บข้อมูลจากดิสก์ โดยหลักการตั้งค่าเบื้องต้นที่แนะนำโดยทั่วไปคือ ประมาณ 25% ของ RAM ทั้งหมด สำหรับเซิร์ฟเวอร์ที่มี RAM      ตั้งแต่ 1GB ขึ้นไป
...
ตอบ ค่าที่อยู่ใน postgresql.auto.conf จะมีผลบังคับใช้ทับค่าเดียวกันใน postgresql.conf เสมอ

5. Buffer hit ratio คืออะไร
ตอบ คือค่าที่บ่งบอกประสิทธิภาพของแคช (Cache) ในฐานข้อมูล โดยคำนวณเป็นเปอร์เซ็นต์เพื่อแสดงว่า บ่อยแค่ไหนที่ฐานข้อมูลค้นหาข้อมูลเจอในหน่วยความจำ (RAM) ที่รวดเร็ว
